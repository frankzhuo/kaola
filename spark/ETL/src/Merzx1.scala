...
class Merzx1 {
  ...
  var data :RDD[(String,String)] = sc.textFile("/user/hive/warehouse/zhengxin.db/mer_tmp",20).map{
    line=>
 ...
 // result.count()
  ...
  resultend.count()
  resultend.cache()
  resultend.saveAsTextFile("/user/root/merzx6")
  ...
  var rDataFrame = hiveContext.createDataFrame(rowRDD, schema1)
  // Register the DataFrames as a table.
  rDataFrame.registerTempTable("merzx_times")
  hiveContext.sql("use zhengxin")
  rDataFrame.insertInto("merzx_times")
