...
@Crawler(name = "zyfblbgg", delay = 15)
public class zyfblbggSpider extends BaseSeimiCrawler {
	...
	public String[] startUrls() {
		try {
			file = new BufferedWriter(new FileWriter("d://zyfblfgg.txt", false));
		} catch (IOException e) {
			e.printStackTrace();
		}
		// 初始化页面
		return new String[] { "http://www.ccgp.gov.cn/cggg/zygg/fblbgg/index.htm" };
	}
	...
	public void start(Response response) {
		...
		for (int i = 1; i < 25; i++) {
			...
			push(new Request("http://www.ccgp.gov.cn/cggg/zygg/fblbgg/index_" + i + ".htm", "callback").setSkipDuplicateFilter(false).setHttpMethod(HttpMethod.GET));
		}
	}
	...
	public void callback(Response response) {
		...
		try {
			...
			for (Object tmp : list) {
				String request = "http://www.ccgp.gov.cn/cggg/zygg/fblbgg" + tmp.toString().substring(1);
				try {
					Thread.sleep(1*1000);
				} catch (Exception e) {
					e.printStackTrace();
	...
	public void getData(Response response) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if(tmp.contains("项目编号：") || tmp.contains("项目编码：")){
			...
			String str =projectNum+"\001"+projectName+"\001"+projectContact+"\001"+contact+"\001"
					...
					+fblbDate+"\001"+season+"\001zy\001"+url+"\n";;
