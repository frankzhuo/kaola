...
@Crawler(name = "shixinbaidu_agent_spider_temp",proxy="123.56.139.108:8123")
public class ShixinBaidu_Agent_Spider_Temp extends BaseSeimiCrawler {
    ...
    public String[] startUrls() {
    	try{
    	    ...
    	    fileReader = new BufferedReader(new FileReader("/data/crawler/shixin/courtlosecredited_jiedaibao.txt"));
    	    filew=new FileWriter("/data/crawler/shixin-"+today+"jdb.txt",true);
    	    filec=new FileWriter("/data/crawler/shixinc-"+today+"jdb.txt",true);
    ...
    public void start(Response response) {
        ...
        try {
        	...
        	while((line = fileReader.readLine()) !=null){
	        	...
	        	name = line.split("	")[0];
	        	id = line.split("	")[1].substring(14);
	        	
	        	System.out.println("-------------the request ------------------"+name+id);
	        	push(new Request("https://sp0.baidu.com/8aQDcjqpAAV3otqbppnN2DJv/api.php?resource_id=6899&query=%E5%A4%B1%E4%BF%A1%E8%A2%AB%E6%89%A7%E8%A1%8C%E4%BA%BA%E5%90%8D%E5%8D%95&cardNum="+id+"&iname="+name.trim()+"&areaName=&pn=0&rn=50&&ie=utf-8&oe=utf-8&format=json&t="+new Date().getTime(),"getData").setSkipDuplicateFilter(true));
        	    Thread.sleep(2*1000);
        	}
        } catch (Exception e) {
            e.printStackTrace();
	    ...
	    try {
	            ...
	            for(int i=0;i<list.size();i++){
	            	...
	            	if(list.get(i).get("cardNum").length()>11){//个人,在百度搜索"失信人被执行人"关键字,重起后又继续爬取
		            	String temp = id+"\001"+id+"\001"+list.get(i).get("iname")+"\001"+list.get(i).get("caseCode")+"\001"+list.get(i).get("age")+"\001"+list.get(i).get("sexy")+"\001"
	            		...
	            		String temp = id+"\001"+id+"\001"+list.get(i).get("iname")+"\001"+list.get(i).get("caseCode")+"\001"+list.get(i).get("businessEntity")+"\001"
	    		            	+list.get(i).get("cardNum")+"\001"+list.get(i).get("courtName")+"\001"+list.get(i).get("areaName")+"\001"+list.get(i).get("gistId")+"\001"
	    		            	+list.get(i).get("regDate")+"\001"+list.get(i).get("gistUnit")+"\001"+list.get(i).get("duty")+"\001"+list.get(i).get("performance")+"\001"
	    		            	+list.get(i).get("disruptTypeName")+"\001"+list.get(i).get("publishDate")+"\001"+list.get(i).get("_update_time")+"\001"+list.get(i).get("partyTypeName")+"\n\r";
	    		        filec.write(temp);
