...
a.cardno,--卡号
(case when b.bank_name is null then c.bank_name  else  b.bank_name  end ) as bank_name,--卡银行
...
group by mobile_num,cardno,bank_name ) b  on  a.mobile_num=b.mobile_num  and  a.cardno=b.cardno
left join (
select  
mobile_num,
cardno,
...
group by  mobile_num,cardno,bank_name) c  on   a.mobile_num=c.mobile_num  and  a.cardno=c.cardno;
EOF
tmp=$?
NOWTIME=$(date +%Y-%m-%d/%H:%M:%S)
if test $tmp -eq 0; then
