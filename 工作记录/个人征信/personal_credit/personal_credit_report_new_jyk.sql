...
sum(total_am)/(100*count(1)) as bj_am --笔均金额
from  hdm.f_fact_trans_success_new_details
where length(mobile_num)=11 and ymd >='20140712'  
group by mobile_num,
(case when tcat_lv5_name='公缴费-电力'  then  '电费'
when tcat_lv5_name='公缴费-水'  then  '水费'
when tcat_lv5_name='公缴费-煤气'  then  '燃气费'
...
creditck.t_public_payment_list b where a.mobile_num=b.mobile_num limit 100;
EOF
tmp=$?
NOWTIME=$(date +%Y-%m-%d/%H:%M:%S)
if test $tmp -eq 0; then
...
from   hdm.f_fact_trans_success_new_details  
where  length(mobile_num)=11 and ymd >='20140712'  and  tcat_lv3_name <> '查询' 
union all
...
left join zhengxin.t_top3_card_trans_new t2   on  t1.mobile_num=t2.mobile_num limit 10;
EOF
tmp=$?
NOWTIME=$(date +%Y-%m-%d/%H:%M:%S)
if test $tmp -eq 0; then
...
b.debit_cxmonth,--借记卡查询月份数
b.credit_cxmonth,--信用卡查询月份数
...
left join creditck.var_trans b on a.mobile_num=b.mobile_num   -- 换成creditck.var_trans
left join creditck.var_cardno_lv_cnt c on a.mobile_num=c.mobile_num
left join (select mobile as mobile_num,'1' as zdfq_count from hds.rep_ccd_bill where trade_time>='20160112' group by mobile) f on a.mobile_num=f.mobile_num
...
left join hds.zy_store_common t1 on t.s_no=t1.s_no
where t.ymd>='20140712'  and  t1.address <>'1'
...
left join  creditck.t_public_payment_all  i  on  a.mobile_num=i.mobile_num limit 10;
EOF
tmp=$?
NOWTIME=$(date +%Y-%m-%d/%H:%M:%S)
if test $tmp -eq 0; then
...
from hds.mq_jf_car_his) t  where row_num=1) a,
hds.u_user  b  ---hds.nuc_ecusr
where a.user_id=b.id )  t3   on t1.mobile_num=t3.mobile_num limit 10;
EOF
tmp=$?
NOWTIME=$(date +%Y-%m-%d/%H:%M:%S)
if test $tmp -eq 0; then
