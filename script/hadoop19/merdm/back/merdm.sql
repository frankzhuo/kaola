...
DATE_0m=$(date +%Y-%m-)01
#DATE_0M=$(date +%Y%m)01
DATE_0M=$(date --date='5 month ago' +%Y%m)01
...
select mer_id,           
 ...
 orgid,substr(ymd,1,6) as mon
    ...
    from (
      ...
      where  ymd>='${DATE_2Y}' and ymd<'${DATE_0M}' and trans_amt>0 
      ---union all
...
select 
       ...
       orgid,substr(ymd,1,6) as mon
    from (
      select * from hdw.lklpos_atmtxnjnl where trans_amt>0 and ymd>='${DATE_2Y}' and ymd<'${DATE_0M}'
      ---union all
      ---select * from hdw.wbpos_atmtxnjnl where trans_amt>0 and ymd>='${DATE_2Y}' and ymd<'${DATE_0M}'
    ) a 
...
from (
  select *
  from hdw.lklpos_atmtxnjnl where trans_amt>0 and ymd>='${DATE_2Y}' and ymd<'${DATE_0M}'
  ---union all
  ---select *
  ---from hdw.wbpos_atmtxnjnl where trans_amt>0 and ymd>='${DATE_2Y}' and ymd<'${DATE_0M}' 
  ) t
group by orgid,mer_id,substr(ymd,1,6);  
