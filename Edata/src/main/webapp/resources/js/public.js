...
//新打开模态窗口
	function openModalDialog(winUrl,param,height,width,left,top,scroll)
	{
		var center="yes";// {yes | no | 1 | 0 }：窗口是否居中，默认yes，但仍可以指定高度和宽度。
		var help="no";//help: {yes | no | 1 | 0 }：是否显示帮助按钮，默认yes。
		var resizable="no";// {yes | no | 1 | 0 } ［ＩＥ５＋］：是否可被改变大小。默认no。
		var status="no";//{yes | no | 1 | 0 } ［IE5+］：是否显示状态栏。默认为yes[ Modeless]或no[Modal]。
		//var scroll="no";//{ yes | no | 1 | 0 | on | off }：指明对话框是否显示滚动条。默认为no。
	
		var strFeatures="";
		strFeatures=strFeatures+"center:" + center + ";";
		strFeatures=strFeatures+"scroll:" + scroll + ";";
		strFeatures=strFeatures+"help:" + help + ";";
		strFeatures=strFeatures+"resizable:" + resizable + ";";
		strFeatures=strFeatures+"status:" + status + ";";
		
		strFeatures=strFeatures+"dialogWidth:" + width + "px;";
		strFeatures=strFeatures+"dialogHeight:" + height + "px;";
		strFeatures=strFeatures+"dialogLeft:" + left + "px;";
		strFeatures=strFeatures+"dialogTop:" + top + "px;";
		if(winUrl.indexOf('?')>0){
			winUrl = winUrl + "&r_m_a=" + Math.random()
		}else{
			winUrl = winUrl + "?r_m_a=" + Math.random()
		}
		...
		{  
			var returnMessage=window.showModalDialog(winUrl,param,strFeatures);
			if(returnMessage&&returnMessage.reload) {
		...
		{
		    if(openWindowChild(winUrl,param,height,width,left,top,"yes",scroll))
		    {
		    	window.location.reload();
...
//联动数据 引入prototype才可用
function getData(field, target, type, children) {
	var url = "linkage/linkage";
	var params = {"id":field.value, "timestamp":new Date().getTime(), "type":type};
	new Ajax.Request(
    	url, {
        method: 'post',
        parameters: params,
        onComplete : function(response) {
        	...
        	for(var i = 0; i < list.length; i++) {
        		opt = new Option(list[i].name, list[i].id);
        		$(target).options.add(opt);
        	}
        	if(children != null) {
        		for(var i = 0; i < children.length; i++) {
        			$(children[i]).options.length = 0;
        			$(children[i]).options.add(new Option('--请选择--',''));
        		}
        	}
        }
    });
...
function selectPageSize(field) {
	document.location.href = document.all.firstPageUrl.href + "&pageSize="
			+ field.value;
}
	...
	if (confirm(c)) {
		return window
				.showModalDialog(
						url,
						"window123",
						"dialogHeight:234px;dialogWidth:271px;resizable:no;help:yes;status:no;scroll:no");
	}
	return false;
}
