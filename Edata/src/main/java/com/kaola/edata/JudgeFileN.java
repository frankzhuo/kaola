...
/*	  	if(termList.get(i).nature.name().equals("nr")||termList.get(i).nature.name().equals("nt")){
...
//	         		System.out.println("fail***"+str);
	...
	public static void main(String[] args) {
		...
		try{
		    ...
		    File file = new File("E:\\crawler\\judgement\\washed_judicial_11.txt.COMPLETED");
		    //File file = new File("");
	        BufferedReader reader = null;
	        FileWriter fileWritter = new FileWriter("d:\\e7.txt",true);
	        ...
	        try {
	           ...
	           // Segment segment = new CRFSegment().enableCustomDictionary(false);
	            ...
	            while ((tempString = reader.readLine()) != null) {
	            	//tempString="杭州经济技术开发区人民法院民 事 裁 定 书（2014）杭经开民初字第400号原告：鞠耀明。原告：袁从慧。上述二原告共同委托代理人：邵瑞青，北京德恒（杭州）律师事务所律师。上述二原告共同委托代理人：陈梦雨，北京德恒（杭州）律师事务所律师。被告：杭州宝龙房地产开发有限公司。法定代表人：许健康。委托代理人：沈乐。本院在审理原告鞠耀明、袁从慧诉被告杭州宝龙房地产开发有限公司商品房销售合同纠纷一案中，原告鞠耀明、袁从慧于2014年7月18日向本院申请撤回对被告杭州宝龙房地产开发有限公司的起诉。本院认为：原告鞠耀明、袁从慧的撤诉申请符合有关法律规定，应予准许。依照《中华人民共和国民事诉讼法》第一百四十五条第一款、第一百五十四条第一款第（五）项之规定，裁定如下：准许原告鞠耀明、袁从慧撤回对被告杭州宝龙房地产开发有限公司的起诉。案件受理费50元，减半收取25元，由原告鞠耀明、袁从慧负担。审判员　管　媛二〇一四年七月十八日书记员　李良峰";
...
/*	            	//审理程序
	            	...
	            	//各当事人抽取
	            	for(int i=0;i<str1.length&&i<10;i++){
	            		tmp1=str1[i];
...
//			                	continue;
			                ...
			                if(douhao>-1)
			                	yuangao=yuangao +" "+tmp1.substring(0,douhao) ;
			                else 
			                	yuangao=yuangao +" "+tmp1;
	            			
			                yuangao=yuangao.replaceAll("人", "");
			                yuangao=yuangao.replaceAll("：", "");
			                yuangao=yuangao.replaceAll("。", "");
			                if(yuangao.length()>5)
			                	yuangao=getPerson(yuangao,segment);
			                dangshiren=dangshiren+yuangao+" ";
			                dangshirenAll=dangshirenAll+yuangao+":"+getBirthdaySex(tmp1)+" ";
	            		...
	            		if((tmp1.startsWith("申诉人")||tmp1.startsWith("上诉人")||tmp1.startsWith("再审申请人")||
	            			tmp1.startsWith("申请人")||tmp1.startsWith("申请执行人") ||tmp1.startsWith("复议申请人"))&&shangshuren.equals("")){
			                ...
			                if(tmp1.indexOf("地址")!=-1&&(tmp1.indexOf("地址")<douhao||douhao==-1))
			                	douhao=tmp1.indexOf("地址");
			                if(douhao>-1)
			                	shangshuren=shangshuren+" "+tmp1.substring(0,douhao) ;
			                else 
			                	shangshuren=shangshuren+" "+tmp1;
			                
			                
			                shangshuren=shangshuren.replaceAll("：", "");
			                shangshuren=shangshuren.replaceAll("。", "");
			                
			                if(shangshuren.length()>11)
			                	shangshuren=getPerson(shangshuren,segment);
			                //shangshuren=shangshuren.replaceAll("(*)", "");
			                //shangshuren=shangshuren.replaceAll("（.*）", "");
			                
			                dangshiren=dangshiren+shangshuren+" ";
			                dangshirenAll=dangshirenAll+shangshuren+":"+getBirthdaySex(tmp1)+" ";
			            	//System.out.println("all:"+dangshirenAll);
	            		...
	            		/*
	               		if(shenqingren.equals("")&&(tmp1.startsWith("申请人")||tmp1.startsWith("申请执行人") ||tmp1.startsWith("复议申请人"))){
	                        int start=3;
	                        if(tmp1.startsWith("申请执行人")||tmp1.startsWith("复议申请人")){
	                        	start=5;
	                        }
	               			tmp1=tmp1.substring(start, tmp1.length());
			                int  douhao=-1;
			                douhao=tmp1.indexOf(",");
			                if(tmp1.indexOf("，")!=-1&&(tmp1.indexOf("，")<douhao||douhao==-1))
			                	douhao=tmp1.indexOf("，");
			                ...
			                if(tmp1.indexOf("地址")!=-1&&(tmp1.indexOf("地址")<douhao||douhao==-1))
			                	douhao=tmp1.indexOf("地址");
			                if(douhao>-1)
			                	shenqingren=shenqingren+" "+tmp1.substring(0,douhao) ;
			                else 
			                	shenqingren=shenqingren+" "+tmp1;
			                shenqingren=shenqingren.replaceAll("。", "");
			                shenqingren=shenqingren.replaceAll("：", "");
			              
			                if(shenqingren.length()>11)
			                	shenqingren=getPerson(shenqingren,segment);

			                weitoFlag="申请人";
			                dangshiren=dangshiren+shenqingren+" ";
			                dangshirenAll=dangshirenAll+shenqingren+":"+getBirthdaySex(tmp1)+" ";
			       
			                
	            		}
	               		*/
	               		if((tmp1.startsWith("被告")||tmp1.startsWith("罪犯"))&&beigao.equals("")){
			                ...
			                if(tmp1.indexOf("地址")!=-1&&(tmp1.indexOf("地址")<douhao||douhao==-1))
			                	douhao=tmp1.indexOf("地址");
			                if(douhao>-1)
			                	beigao=beigao+" "+tmp1.substring(0,douhao) ;
			                else 
			                	beigao=beigao+" "+tmp1;

			                beigao=beigao.replaceAll("人", "");
			                beigao=beigao.replaceAll("。", "");
			                beigao=beigao.replaceAll("：", "");
			                if(beigao.length()>11)
			                	beigao=getPerson(beigao,segment);

			                dangshiren=dangshiren+beigao+" ";
			                dangshirenAll=dangshirenAll+beigao+":"+getBirthdaySex(tmp1)+" ";
			            	//System.out.println("all:"+dangshirenAll);
			            	weitoFlag="被告";

			                
	            		}
	               		if((tmp1.startsWith("被执行人")||tmp1.startsWith("被上诉人")||tmp1.startsWith("被申请人")||tmp1.startsWith("被申诉人"))&&beishangshuren.equals("")){
			                ...
			                if(tmp1.indexOf("地址")!=-1&&(tmp1.indexOf("地址")<douhao||douhao==-1))
			                	douhao=tmp1.indexOf("地址");
			                if(douhao>-1)
			                	beishangshuren=beishangshuren+" "+tmp1.substring(0,douhao) ;
			                else 
			                	beishangshuren=beishangshuren+" "+tmp1;
			                beishangshuren=beishangshuren.replaceAll("。", "");
			                beishangshuren=beishangshuren.replaceAll("：", "");
			                if(beishangshuren.length()>11)
			                	beishangshuren=getPerson(beishangshuren,segment);
			        
			                dangshiren=dangshiren+beishangshuren+" ";
			                dangshirenAll=dangshirenAll+beishangshuren+":"+getBirthdaySex(tmp1)+" ";
			            	//System.out.println("all:"+dangshirenAll);
			            	weitoFlag="被上诉人";		                
	            		}
	            		/*
	               		if(tmp1.startsWith("被执行人")&&beizhixinren.equals("")){			
	                        tmp1=tmp1.substring(4, tmp1.length());
	                        //System.out.println("*"+tmp1);
			                int  douhao=-1;
			                douhao=tmp1.indexOf(",");
			                ...
			                if(tmp1.indexOf("地址")!=-1&&(tmp1.indexOf("地址")<douhao||douhao==-1))
			                	douhao=tmp1.indexOf("地址");
			                if(douhao>-1)
			                	beizhixinren=beizhixinren+" "+tmp1.substring(0,douhao) ;
			                else 
			                	beizhixinren=beizhixinren+" "+tmp1;
			                beizhixinren=beizhixinren.replaceAll("。", "");
			                beizhixinren=beizhixinren.replaceAll("：", "");
			                if(beizhixinren.length()>11)
			                	beizhixinren=getPerson(beizhixinren,segment);

			                dangshiren=dangshiren+beizhixinren+" ";
			                dangshirenAll=dangshirenAll+beizhixinren+":"+getBirthdaySex(tmp1)+" ";			          	
			            	weitoFlag="被执行人";
	            	...
	            	if(yuangao.equals("")&&shangshuren.equals("")&&shenqingren.equals("")&&beigao.equals("")&&beishangshuren.equals("")&&beizhixinren.equals(""))
	            		System.out.println(mflag+"***************原文2:"+merge);
	            	//System.out.println("");
	            	//判决时间	           
	            	if(judgeDate.equals("")){
